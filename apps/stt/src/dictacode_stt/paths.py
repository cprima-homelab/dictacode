"""Canonical paths for dictacode STT.
...
logger = logging.getLogger("dictacode.stt.paths")
...
STT_CONFIG_FILE = Path(
    os.getenv("DICTACODE_STT_CONFIG", str(_CONFIG_BASE / "stt.conf"))
)
STT_DROP_IN_DIR = Path(
    os.getenv("DICTACODE_STT_DROP_IN_DIR", str(_CONFIG_BASE / "stt.d"))
)

# === Audio ===
AUDIO_CONFIG_DIR = Path(
    os.getenv("DICTACODE_AUDIO_CONFIG_DIR", str(_CONFIG_BASE / "audio"))
)
AUDIO_PROFILES_DIR = Path(
    os.getenv("DICTACODE_AUDIO_PROFILES_DIR", str(_CONFIG_BASE / "audio/profiles"))
)
...
CANONICAL_PATHS = {
    ...
    "darwin": {
        "stt_config": "/Library/Application Support/dictacode/stt.conf",
        "stt_drop_in": "/Library/Application Support/dictacode/stt.d",
        "audio_config_dir": "/Library/Application Support/dictacode/audio",
        "audio_profiles_dir": "/Library/Application Support/dictacode/audio/profiles",
...
def find_audio_config_dir(override: Optional[str] = None) -> Path:
    """Find audio config directory."""
    return find_asset(
        "audio config",
        override,
        "DICTACODE_AUDIO_CONFIG_DIR",
        [str(AUDIO_CONFIG_DIR)],
    )


def find_audio_profiles_dir(override: Optional[str] = None) -> Path:
    """Find audio profiles directory."""
    return find_asset(
        "audio profiles",
        override,
        "DICTACODE_AUDIO_PROFILES_DIR",
        [str(AUDIO_PROFILES_DIR)],
    )
