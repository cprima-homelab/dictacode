...
class TestDiagnosticResult:
    ...
    def test_to_json(self):
        ...
        assert parsed["component"] == "stt"

    def test_to_human_basic(self):
        """Test human-readable output."""
        result = DiagnosticResult(component="stt")
        result.ok("audio_device", "Device found")
        result.fail("whisper_model", "Model not found")

        output = result.to_human()
        assert "=== dictacode STT Hardware Check ===" in output
        assert "[OK]" in output
    ...
    def test_to_human_quiet(self):
        ...
        result = DiagnosticResult(component="stt")
        result.ok("audio_device", "OK")
        result.fail("whisper_model", "Failed", next_step="Download model")

        output = result.to_human(quiet=True)
        assert "[OK]" not in output
        assert "[FAIL]" in output
        assert "whisper_model" in output
