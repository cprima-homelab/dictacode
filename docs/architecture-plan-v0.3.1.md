# dictacode Architecture Plan v0.3.1 - LLM Post-Processing & Command Detection

## Status

### Phase 1: LLM Adapter Interface
- [x] Create `llm/` package
- [x] Define `LlmAdapter` ABC
- [x] Define `LlmRequest`, `LlmResponse` types
- [x] Define `ProcessingProfile` for prompt templates
...
- [x] Define profile schema (system prompt, user template)
- [x] Implement profile loading from config
- [x] Built-in profiles: grammar, punctuation, formal, casual, code, passthrough
- [x] Profile selection via CLI/config
- [ ] Unit tests for profile application

### Phase 4: Pipeline Integration
- [x] Create `PostProcessor` class
- [x] Integrate into `SttService` pipeline
- [x] Optional bypass (passthrough mode)
...
**v0.3.1 IN PROGRESS** - Week 1 complete (adapters, profiles, service integration)

---

## Prerequisites

v0.3.1 builds on top of:
